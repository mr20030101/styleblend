...
<head>
    ...
    <title>Batch Barcode Print — Track Buddy</title>
...
<div class="no-print bg-white border-b border-gray-200 px-6 py-3 flex items-center gap-4 sticky top-0 z-20 shadow-sm">
    <a href="{{ route('products.index') }}" class="text-gray-400 hover:text-gray-700 transition">
        <i class="fas fa-arrow-left"></i>
    </a>
    <div class="flex-1 min-w-0">
        <h1 class="font-bold text-gray-900 text-base truncate">Batch Barcode Print</h1>
        <p class="text-xs text-gray-400">Select products and print all labels at once</p>
    </div>
...
<div class="no-print flex h-[calc(100vh-57px)]">
    ...
    {{-- Left sidebar --}}
    <div class="w-72 flex-shrink-0 bg-white border-r border-gray-200 flex flex-col overflow-hidden">
        <div class="p-4 space-y-4 overflow-y-auto flex-1">

            {{-- Copies --}}
            <div>
                <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Copies per Item</label>
            ...
            <div>
                ...
                <div class="space-y-1.5">
                    @foreach(['50x30' => '50 × 30 mm', '60x40' => '60 × 40 mm', '40x25' => '40 × 25 mm (small)'] as $val => $lbl)
                    <label class="flex items-center gap-2.5 cursor-pointer p-2 rounded-lg hover:bg-gray-50 transition">
                        <input type="radio" name="label-size" value="{{ $val }}" {{ $val === '50x30' ? 'checked' : '' }}
                            onchange="renderLabels()" class="text-brand">
            ...
            <div>
                ...
                <label class="flex items-center justify-between p-2 rounded-lg hover:bg-gray-50 cursor-pointer transition">
                    ...
                    <input type="checkbox" id="show-price" checked onchange="renderLabels()" class="rounded border-gray-300">
                </label>
                <label class="flex items-center justify-between p-2 rounded-lg hover:bg-gray-50 cursor-pointer transition">
                    <span class="text-sm text-gray-700">Show Store Name</span>
                    <input type="checkbox" id="show-store" checked onchange="renderLabels()" class="rounded border-gray-300">
                </label>
                <label class="flex items-center justify-between p-2 rounded-lg hover:bg-gray-50 cursor-pointer transition">
                    <span class="text-sm text-gray-700">Show Variant</span>
                    <input type="checkbox" id="show-variant" checked onchange="renderLabels()" class="rounded border-gray-300">
                </label>
            </div>
            ...
            <div>
                <div class="flex items-center justify-between mb-2">
                    <label class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Products</label>
                ...
                {{-- Search filter --}}
                <input type="text" id="product-search" placeholder="Filter products..."
                    class="w-full border border-gray-200 rounded-lg px-3 py-1.5 text-xs mb-2 focus:outline-none focus:ring-2 focus:ring-brand"
                    oninput="filterProducts()">

                <div class="space-y-1" id="product-list">
                    @foreach($products as $product)
                    <label class="flex items-start gap-2.5 p-2 rounded-lg hover:bg-gray-50 cursor-pointer transition product-item"
                        data-name="{{ strtolower($product['name']) }}">
                        <input type="checkbox" class="product-check mt-0.5 rounded border-gray-300"
                            value="{{ $product['id'] }}" checked onchange="renderLabels()">
                        <div class="min-w-0 flex-1">
                            <p class="text-xs font-semibold text-gray-800 leading-tight">{{ $product['name'] }}</p>
                            <p class="text-xs text-gray-400">{{ $product['category'] }} · {{ count($product['variants']) }} variant{{ count($product['variants']) !== 1 ? 's' : '' }}</p>
                        </div>
                    </label>
                    @endforeach
                </div>
    ...
    <div class="flex-1 overflow-y-auto p-6 bg-gray-100">
        ...
        <p id="empty-msg" class="hidden text-center text-gray-400 mt-20 text-sm">No products selected.</p>
    </div>
</div>
...
function getSelectedIds() {
    return [...document.querySelectorAll('.product-check:checked')].map(el => parseInt(el.value));
}

function toggleAll(state) {
    document.querySelectorAll('.product-check').forEach(el => el.checked = state);
    renderLabels();
}
...
function makeLabel(productName, variant, wMm, hMm) {
    ...
    selected.forEach(product => {
        product.variants.forEach(variant => {
            for (let i = 0; i < copies; i++) {
                preview.appendChild(makeLabel(product.name, variant, w, h));
                print.appendChild(makeLabel(product.name, variant, w, h));
                total++;
            }
        });
    });
